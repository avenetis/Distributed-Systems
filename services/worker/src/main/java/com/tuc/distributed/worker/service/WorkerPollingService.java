...
public class WorkerPollingService {
    ...
    // Register with manager when Spring Boot finishes starting
    ...
    public void pollForTasks() {
        ...
        try {
            ...
            // 204 : no tasks avail (no content is normal)
            if(!e.getMessage().contains("204")) {
                logger.debug("Poll cycle, to tasks or error: {}", e.getMessage());
            }
        }
    }
    ...
    private void executeTask(ClaimTaskResponse claimedTask) {
        TaskRequest request = new TaskRequest(claimedTask.getTaskId(), claimedTask.getJobId(), claimedTask.getTaskType().name().equals("MAP") ?
